...
log_cm
```
Logistic Regression Model A achieved an accuracy of 82.28%. However, despite the relatively high accuracy, the model performed poorly in identifying delayed flights, achieving a recall of only 1.00%. In contrast, specificity reached 99.82%, indicating that the model primarily predicted the majority class (OnTime). This behaviour is reflected in the low balanced accuracy (50.41%) and Cohen's Kappa (0.0134). Overall, Logistic Regression Model A served as a useful baseline but was ineffective for detecting flight delays in this dataset.
...
Random Forest Model A achieved an accuracy of 84.19%, outperforming Logistic Regression Model A (82.28%). The model recorded a precision of 69.10% and a recall of 19.78%, demonstrating a substantially improved ability to identify delayed flights compared to Logistic Regression, which achieved a recall of only 1.00%.

The balanced accuracy increased to 58.94%, indicating improved performance across both classes while maintaining high specificity (98.09%). These results suggest that Random Forest was better able to capture nonlinear relationships and interactions among operational, temporal, geographical, and weather-related variables, resulting in superior classification performance when departure delay information was excluded.
...
log_cm_B
```
Model B achieved an accuracy of 93.42%, substantially improving upon Logistic Regression Model A (82.28%). The inclusion of departure delay provided significant predictive power, enabling the model to better distinguish between delayed and on-time flights.

The model achieved a recall of 71.35% and a precision of 89.43%, indicating strong performance in identifying delayed flights while maintaining a low rate of false positives. Specificity remained high at 98.18%, demonstrating excellent performance in correctly classifying on-time flights. The balanced accuracy of 84.76% and Cohen's Kappa of 0.7551 further indicate strong classification performance across both classes.
...
The model achieved an accuracy of 93.93%, representing the highest classification performance among all models evaluated. It also achieved a precision of 91.07% and a recall of 72.96%, indicating strong performance in identifying delayed flights while maintaining a low false-positive rate. Furthermore, the model achieved a specificity of 98.46%, demonstrating excellent performance in correctly classifying on-time flights. The balanced accuracy of 85.71% and Cohen's Kappa value of 0.7745 further confirm strong classification performance across both classes.

Compared with Logistic Regression Model B, Random Forest Model B produced slightly higher accuracy, recall, balanced accuracy, and Kappa, suggesting that nonlinear relationships and feature interactions contribute additional predictive value beyond what can be captured by a linear model. The results indicate that combining operational, temporal, route, weather, and departure delay information with an ensemble learning approach provides the most effective solution for flight delay classification in this study.
...
comparison_table <- data.frame(
  ...
  Accuracy = c(82.28, 84.19, 93.42, 93.93),
  Recall = c(1.00, 19.78, 71.35, 72.96),
  Specificity = c(99.82, 98.09, 98.18, 98.46),
  Balanced_Accuracy = c(50.41, 58.94, 84.76, 85.71)
)
...
```
Among the four classification models evaluated, Random Forest Model B achieved the best overall performance, with an accuracy of 93.93%, recall of 72.96%, specificity of 98.46%, and balanced accuracy of 85.71%. The inclusion of departure delay substantially improved predictive performance across both Logistic Regression and Random Forest models, highlighting its importance as a predictor of arrival delays.
...
```{r load-data}
df_model <- readRDS("cleaned_flights.rds")
